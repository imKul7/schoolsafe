...
import { type FormEventHandler, type MouseEvent as ReactMouseEvent, type ReactNode } from 'react';
...
type StudentField = 'school_class_id' | 'student_number' | 'nisn' | 'full_name' | 'gender' | 'date_of_birth' | 'status' | 'notes';
...
export default function EditStudent({ student, classes }: EditStudentProps) {
    const { data, setData, put, processing, errors, clearErrors, isDirty } = useForm<StudentForm>({
    ...
    const updateField = (field: StudentField, value: string): void => {
        setData(field, value);
    ...
    const confirmLeave = (event: ReactMouseEvent<Element>): void => {
        if (!isDirty) {
            return;
        }

        const confirmed = window.confirm('Perubahan yang belum disimpan akan hilang. Tetap tinggalkan halaman?');
            ...
            <main className="min-h-full bg-[#f8fafc]">
                <div className="mx-auto w-full max-w-5xl p-4 md:p-6">
                    <Link
                        ...
                        className="mb-5 inline-flex items-center gap-2 rounded-lg text-sm font-semibold text-[#627d98] transition hover:text-[#4f7cac] focus-visible:ring-2 focus-visible:ring-[#dcebf8] focus-visible:outline-none"
                    >
                    ...
                    <section className="relative mb-6 overflow-hidden rounded-[26px] border border-[#deebf5] bg-gradient-to-r from-[#edf6ff] via-[#f2faf8] to-[#fffaf0] p-6 shadow-sm md:p-8">
                        ...
                        <div className="relative flex flex-col justify-between gap-5 sm:flex-row sm:items-center">
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    <h1 className="text-2xl font-bold text-[#243b53]">Edit Data Siswa</h1>

                                    <p className="mt-1 text-sm leading-6 text-[#627d98]">
                                        Perbarui identitas dan informasi akademik {student.full_name}.
                                    </p>
                                </div>
                            </div>
                    ...
                    {errorMessages.length > 0 && (
                        <section role="alert" aria-live="assertive" className="mb-6 rounded-2xl border border-[#f0cece] bg-[#fff4f4] p-4">
                            <div className="flex items-start gap-3">
                                <AlertCircle aria-hidden="true" className="mt-0.5 size-5 shrink-0 text-[#cf6464]" />

                                <div>
                                    <h2 className="text-sm font-semibold text-[#a64f4f]">Data belum dapat disimpan</h2>

                                    <p className="mt-1 text-sm leading-6 text-[#b46363]">Periksa kembali kolom yang ditandai merah.</p>
                                </div>
                            </div>
                        </section>
                    )}
                    ...
                    <form onSubmit={submit} className="space-y-6" aria-busy={processing} noValidate>
                        <section className="rounded-2xl border border-[#e6eef5] bg-white p-5 shadow-sm md:p-7">
                            <SectionHeader icon={UserRound} title="Identitas Siswa" description="Informasi dasar dan identitas resmi siswa." />
                            ...
                            <div className="grid gap-5 md:grid-cols-2">
                                <div className="md:col-span-2">
                                    ...
                                    <input
                                        ...
                                        onChange={(event) => updateField('full_name', event.currentTarget.value)}
                                        disabled={processing}
                                        autoComplete="name"
                                ...
                                <div>
                                    <FormLabel htmlFor="student_number" required description="Nomor siswa harus unik dalam sekolah.">
                                        Nomor siswa
                                    </FormLabel>
                                    ...
                                    <div className="relative">
                                        ...
                                        <input
                                            ...
                                            onChange={(event) => updateField('student_number', event.currentTarget.value)}
                                            disabled={processing}
                                            autoComplete="off"
                                ...
                                <div>
                                    <FormLabel htmlFor="nisn" description="Isi tepat 10 angka bila tersedia.">
                                        NISN
                                    </FormLabel>
                                    ...
                                    <div className="mt-1 flex items-start justify-between gap-3">
                                        ...
                                        <span className="ml-auto shrink-0 text-xs text-[#9fb3c8]">{data.nisn.length}/10</span>
                                    </div>
                                </div>
                                ...
                                <div>
                                    ...
                                    <div className="relative">
                                        ...
                                        <input
                                            ...
                                            onChange={(event) => updateField('date_of_birth', event.currentTarget.value)}
                                            disabled={processing}
                        ...
                        <section className="rounded-2xl border border-[#e6eef5] bg-white p-5 shadow-sm md:p-7">
                            ...
                            <div className="grid gap-5 md:grid-cols-2">
                                <div>
                                    <FormLabel htmlFor="school_class_id" required>
                                        Kelas
                                    </FormLabel>
                                    ...
                                    <select
                                        ...
                                        onChange={(event) => updateField('school_class_id', event.currentTarget.value)}
                                        disabled={processing || classes.length === 0}
                                        aria-invalid={Boolean(errors.school_class_id)}
                                ...
                                <div className="md:col-span-2">
                                    <FormLabel htmlFor="notes" description="Catatan hanya dapat dilihat petugas sekolah.">
                                        Catatan
                                    </FormLabel>
                                    ...
                                    <div className="mt-1 flex items-start justify-between gap-4">
                                        ...
                                        <span className="ml-auto shrink-0 text-xs text-[#9fb3c8]">{data.notes.length}/2000</span>
                                    </div>
                                </div>
                            </div>
                        </section>
                        ...
                        <section className="sticky bottom-4 z-10 flex flex-col-reverse gap-3 rounded-2xl border border-[#dfe9f1] bg-white/95 p-4 shadow-[0_15px_45px_rgba(50,80,110,0.12)] backdrop-blur sm:flex-row sm:items-center sm:justify-between">
                            <div className="text-xs text-[#829ab1]">
                                {isDirty ? 'Terdapat perubahan yang belum disimpan.' : 'Belum ada perubahan pada data siswa.'}
                            </div>
                            ...
                            <div className="flex flex-col-reverse gap-3 sm:flex-row">
                                ...
                                <Button
                                    type="submit"
                                    disabled={processing || !isDirty || classes.length === 0}
...
function FormLabel({ htmlFor, children, required = false, description }: FormLabelProps) {
    return (
        <div className="mb-2">
            <label htmlFor={htmlFor} className="block text-sm font-semibold text-[#334e68]">
                {children}

                {required && (
                    <span aria-hidden="true" className="ml-1 text-[#e97a7a]">
                        *
                    </span>
                )}
            </label>

            {description && <p className="mt-1 text-xs leading-5 text-[#9fb3c8]">{description}</p>}
        </div>
    );
}

function SectionHeader({ icon: Icon, title, description, tone = 'blue' }: SectionHeaderProps) {
    const iconStyle = tone === 'green' ? 'bg-[#eef9f6] text-[#4c9e94]' : 'bg-[#eef6ff] text-[#5b8def]';

    return (
        <div className="mb-6 flex items-center gap-3">
            <div className={`flex size-10 shrink-0 items-center justify-center rounded-xl ${iconStyle}`}>
                <Icon aria-hidden="true" className="size-5" />
            </div>

            <div>
                <h2 className="font-bold text-[#243b53]">{title}</h2>

                <p className="text-sm leading-6 text-[#829ab1]">{description}</p>
            </div>
        </div>
    );
}
