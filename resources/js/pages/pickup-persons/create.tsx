...
export default function CreatePickupPerson({ students = [] }: CreatePickupPersonProps) {
    const { data, setData, post, processing, errors, clearErrors } = useForm<PickupPersonForm>({
        full_name: '',
        identity_number: '',
        phone: '',
        email: '',
    ...
    const updateStudentLink = <K extends keyof StudentLinkForm>(index: number, field: K, value: StudentLinkForm[K]): void => {
        ...
        clearErrors(`students.${index}.${String(field)}` as keyof PickupPersonForm);
    };
    ...
    const removeStudentLink = (index: number): void => {
        ...
        setData(
            'students',
            data.students.filter((_, studentIndex) => studentIndex !== index),
        );
    };
                ...
                <div className="mx-auto w-full max-w-6xl p-4 md:p-6">
                    ...
                    <section className="relative mb-6 overflow-hidden rounded-[28px] border border-[#deebf5] bg-gradient-to-r from-[#edf6ff] via-[#f2faf8] to-[#fffaf0] p-6 shadow-sm md:p-8">
                        ...
                        <div className="relative flex items-start gap-4">
                            ...
                            <div>
                                <h1 className="text-2xl font-bold text-[#243b53] md:text-3xl">Tambah Penjemput</h1>

                                <p className="mt-2 max-w-2xl text-sm leading-6 text-[#627d98]">
                                    Lengkapi identitas penjemput dan hubungkan dengan siswa yang boleh dijemput.
                                </p>
                            </div>
                        </div>
                    </section>
                        ...
                        <section className="rounded-2xl border border-[#e6eef5] bg-white p-5 shadow-sm md:p-7">
                            <div className="mb-6 flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p className="text-sm text-[#829ab1]">Informasi utama pihak yang diizinkan menjemput siswa.</p>
                                </div>
                            </div>
                                                        ...
                                                        {students.map((student) => (
                                                            <option
                                                                ...
                                                                disabled={data.students.some(
                                                                    (link, linkIndex) =>
                                                                        linkIndex !== index && link.student_id === String(student.id),
                                                                )}
                                                            >
